...
fn main() -> Result<(), Error> {
    ...
    let odom_publisher = node
        .create_publisher::<OdometryMsg>("/odometry/gps", rclrs::QOS_PROFILE_DEFAULT)
        .unwrap();

    let gnss_msg_subscriber_handler = Arc::clone(&gnss_msg_global_handler);
    let _gnss_subscriber = node.create_subscription::<NavSatFixMsg, _>(
        "/gnss",
        rclrs::QOS_PROFILE_DEFAULT,
        move |msg: NavSatFixMsg| {
    ...
    let _mag_subscriber = node.create_subscription::<MagneticFieldMsg, _>(
        "/imu/mag",
        rclrs::QOS_PROFILE_DEFAULT,
        move |msg: MagneticFieldMsg| {
    ...
    let gps_odometry_opt: Option::<gps_odometry::Odometry>;
    loop {
        if let Some(gnss_msg) = &*gnss_msg_global_handler.lock().unwrap() {
            ...
            if let Some(mag_msg) = &*mag_msg_global_handler.lock().unwrap() {
                ...
                // done
                let datum_heading =
                    gps_odometry::get_heading(mag_msg.magnetic_field.x, mag_msg.magnetic_field.y);
